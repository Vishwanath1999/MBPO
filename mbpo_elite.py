...
class EnsembleModel:
    ...
    def get_traj(self,state,action,reward,next_state,batch_size):
...
class SAC_Agent():
    def __init__(self, alpha, beta, input_dims, tau, env,n_elites,
            env_id, gamma=0.99,n_models=10,fake_ratio = 0.95,
            n_actions=2, max_size=int(1e6), layer1_size=256,weight_decay=1e-5,
    ...
    def generate_efficient(self, n_ac_steps=20):
        batch_size = 2000
        self.fake_memory.reallocate(batch_size*n_ac_steps)
        for _ in range(n_ac_steps):
            state,_,reward,next_state,done = self.real_memory.sample_buffer(batch_size)
            ...
            next_state_pred,reward_pred = self.models.get_traj(stateT,actionT,rewardT,next_stateT,batch_size)            
